...
def main() -> None:
    ...
    if schedule_entries:
        min_asof = min(a for _, _, a in schedule_entries)
        max_asof = max(a for _, _, a in schedule_entries)
        min_expiry = min(e for _, e, _ in schedule_entries)
        max_expiry = max(e for _, e, _ in schedule_entries)
    else:
        min_asof = start
        max_asof = end
        min_expiry = start
    ...
    if schedule_entries:
        print(f"[PLAN] asof_range={min_asof}..{max_asof} expiry_range={min_expiry}..{max_expiry}")
    ...
    # Deduplicate row_id collisions (typically caused by asof fallback on non-trading days).
    ...
    if dupes > 0:
        ...
        if "asof_fallback_days" in out_df.columns:
            out_df["__asof_fallback_days"] = pd.to_numeric(out_df["asof_fallback_days"], errors="coerce").fillna(1e9)
            sort_cols.append("__asof_fallback_days")
            ascending.append(True)
        if "expiry_fallback_days" in out_df.columns:
